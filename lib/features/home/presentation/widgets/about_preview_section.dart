...
class AboutPreviewSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final photo = AspectRatio(aspectRatio: 1.1, child: NetworkImagePlaceholder(label: l.aboutPhotoLabel));
    final text = Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Text(l.aboutTitle, style: AppTextStyles.headlineAr.copyWith(fontSize: 32)),
      const SizedBox(height: 18),
      Text(l.aboutParagraph1, style: AppTextStyles.bodyAr.copyWith(fontSize: 15.5)),
      const SizedBox(height: 12),
      Text(l.aboutParagraph2, style: AppTextStyles.bodyAr.copyWith(fontSize: 15.5)),
      const SizedBox(height: 28),
      for (int i = 0; i < figs.length; i++)
        Padding(
          padding: const EdgeInsets.only(bottom: 18),
          child: Row(children: [
            Text('0${i + 1}', style: AppTextStyles.headlineAr.copyWith(fontSize: 22, color: AppColors.gold)),
            const SizedBox(width: 16),
            Expanded(child: Text(figs[i], style: AppTextStyles.bodyAr.copyWith(fontSize: 14))),
          ]),
        ),
    ]);
